...
def rotate_selected_video():
    """ 
    * Should run ffprobe to check if videos are valid prior to deleting old ones..so we only rename good ones
    * should prioritize videos that align with motion detection
    * should make a mode that only saves motion-detected videos
    * export motion detection into prometheus
    """
    
    global selected_cam
    files = sorted([f for f in os.listdir('.') if f.startswith(selected_cam['name']+"-")], key=os.path.getctime)
...
def select_cam(id):
    global selected_cam, cameras
    selected_cam = cameras[id]
    rotate_selected_video()
...
def main():
    ...
    for cam in hub.list_cameras():
        ...
        print(cam)
        select_cam(cam['id'])
        run_ffmpeg_in_background(cam['streaming_url'], cam['name'])
